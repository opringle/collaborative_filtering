...
parser.add_argument('--num-embed', type=int, default=8,
                    help='the user/item latent feature dimension')
parser.add_argument('--fc-layers', type=list, default=[16, 8, 4],
...
def reset_id(df, cols):
    ...
    for col in cols:
        ...
        df[col] = [mapping[old_id] for old_id in df[col].tolist()]
...
def build_iter_data(df, test_interactions, user_feature_cols, item_feature_cols):
    ...
    """
    # Ensure users have atleast 20 interactions
    ...
    val_iter = iterators.SparseNegativeSamplingDataIter(sparse_interactions, user_features, item_features,
                                                          ...
                                                          interaction_label=2, batch_size=args.batch_size)
    ...
    train_iter = iterators.SparseNegativeSamplingDataIter(val_iter.sparse_interactions, user_features, item_features,
                                                          ...
                                                          interaction_label=1, batch_size=args.batch_size)
...
if __name__ == '__main__':
    ...
    # ToDo: user 0 has no train set interactions, user 3 has no interactions!
    train_iter, val_iter, num_users, num_items = build_iter_data(df, test_interactions=args.test_interactions,
